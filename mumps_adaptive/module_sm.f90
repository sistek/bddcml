...
! Local variables
      logical :: present_all, present_any, use_mask, offdiagblock=.false.
...
! Decide according to value of matrixtype which multiplication is appropriate
      ...
      case (0)
         ...
         call sm_vec_mult_full
      case (1,2)
         ! only one triangle of matrix is stored
         call sm_vec_mult_sym
      case default
...
! local variables
      integer:: ia, indi, indj
      logical:: rowmatch, colmatch, multnormal = .false., multtranspose = .false.
...
! Write the matrix on the unit
      write(iunit,'(i8,i8,f10.4)') (i_sparse(ia),j_sparse(ia),a_sparse(ia),ia = 1,nz)
